...
public class Seguim_H : MonoBehaviour
{
    [SerializeField]
    Transform[] waypoints; //se definen puntos de referencia para movimiento horizontal
    
    private float targetTime = 15f; //contador de 40 segundos para duración de la prueba 
    ...
    string csvpath = @"C:\Users\Dani\Documents\PROYECTO INTEGRADOR\CSV_Pruebas\Seguim_H.csv";//direccion del archivo
    ...
    void Update()
    {
        ...
        var coordEstimulo = boton.transform.position; //coordenada del estímulo en sistema de coordenadas world space
        var coordEstimulo_screen = cam.WorldToScreenPoint(boton.transform.position); //coordenada del estímulo en sist. de coordenadas de la pantalla
        var coordGazePoint = gazeData.Screen; //coordenada de la mirada del usuario en la pantalla
